...
class Hexagon{
    ...
    boolean mouse;
    int scale;
    Hexagon(Vector2 hexCenter, int scale) {
        definePoints(hexCenter,scale);
        this.scale = scale;
        this.center = hexCenter;
        this.hexColor = Vector3.green;
        this.wall = false;
        this.mouse = false;
    }
    Hexagon(Vector2 hexCenter, int scale, Vector3 color) {
        definePoints(center, scale);
        this.scale = scale;
        this.hexColor = color;
        this.center = hexCenter;
        this.wall = false;
    ...
    void definePoints(Vector2 hexCenter, int scale) {
        int x = (int)hexCenter.x;
        int y = (int)hexCenter.y;
        this.scale = scale;
        //Array of hexagon coordinates, starting at the middle left and going counterclockwise
        int[] xCoords = {x - 8 * scale, x - 4 * scale, x + 4 * scale, x + 8 * scale, x + 4 * scale, x - 4 * scale};
        int[] yCoords = {y, y + 7 * scale, y + 7 * scale, y, y - 7 * scale, y - 7 * scale};
        
        //Make a polygon object (what we actually draw)
        _poly = new Polygon(xCoords, yCoords, 6);
    }
    void setColor(Vector3 color) {
        this.hexColor = color;
    }
    double proximity(Vector2 pos) {
        double x = pos.x;
        double y = pos.y;
        Vector2 press = _windowPixelFromWorld(pos);
        
        //get mouse position in terms of how polygon is defined
        press.x = (press.x + 1024.0 - 256.0)/4.0;
        press.y = (press.y + 1024.0 - 256.0)/4.0;
        return Math.abs(center.y - press.y) + Math.abs(center.x - press.x);
    }
    
    
    //Taken from app, lets us get world coords in pixels in this class too
    ...
    int _windowTopLeftCornerXInPixels = 0;
    int _windowTopLeftCornerYInPixels = 0;
...
class App extends JPanel {
    ...
    void drawCenterHexagon(Hexagon hex) {
        //Checks for if mouse
        if (hex.mouse) {
            //Draws normal hexagon
            hex.hexColor = Vector3.green;
            _graphicsSetColor(hex.hexColor);
            _graphics.fillPolygon(hex._poly);
            //Draws mouse in hexagon and ends function (so we don't paint over the mouse)
            drawMouse(hex);
            return;
        } else if (hex.wall) {
            hex.hexColor = Vector3.darkGray;
        } else {
            hex.hexColor = Vector3.green;
        }
        //draws hexagons
    ...
    void drawMouse (Hexagon hex) {
        Vector2 center = new Vector2();
        //Main Body
        center.x = (hex.center.x/8.0) - 32.0;
        center.y = (hex.center.y/-8.0) + 32.0;
        drawCircle(center, 0.4 * (double)hex.scale, Vector3.lightGray);
        
        //Move for Nose
        center.y = (hex.center.y/-8.0) + 32.0 - 0.1 * (double)hex.scale;
        drawCircle(center, 0.15 * (double)hex.scale, Vector3.black);
        
        //Move to Left Eye
        center.y = (hex.center.y/-8.0) + 32.0 + 0.15 * (double)hex.scale;
        center.x = ((hex.center.x/8.0) - 32.0) + (0.15 * (double)hex.scale);
        drawCircle(center, 0.075 * (double)hex.scale, Vector3.black);
        ...
        //Move back to center
        center.y = (hex.center.y/-8.0) + 32.0 - 0.05 * (double)hex.scale;
        center.x = (hex.center.x/-8.0) + 32.0 - 0.15 * (double)hex.scale;
        
        //Draw Ears
        center.x = ((hex.center.x/8.0) - 32.0) + (0.4 * (double)hex.scale);
        center.y = ((hex.center.y/-8.0) + 32.0) + (0.4 * (double)hex.scale);
        drawCircle(center, 0.2 * (double)hex.scale, Vector3.lightGray);
        center.x = ((hex.center.x/8.0) - 32.0) - (0.4 * (double)hex.scale);
        drawCircle(center, 0.2 * (double)hex.scale, Vector3.lightGray);
    }
    // input
